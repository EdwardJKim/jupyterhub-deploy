...
function swarm_manage() {
	...
	echo "$SWARM_BINARY" -l debug manage --tls -H "$host" --heartbeat=1s $discovery
	"$SWARM_BINARY" -l debug manage --tls -H "$host" --heartbeat=1s $discovery &
	SWARM_MANAGE_PID[$i]=$!
	SWARM_HOSTS[$i]=$host
